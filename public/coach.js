...
const state = { user: null, config: null, athletes: [], athlete: null, selectedWeekStart: null, editingSession: -1, modalBlocks: [], activities: [], recovery: [], currentActivityId: null, currentActivity: null };
...
function mondayFromDateString(dateString) {
  if (!dateString) return isoMonday();
  return isoMonday(new Date(`${dateString}T12:00:00`));
}
function weekRangeLabel(weekStart) {
  const end = addDays(weekStart, 6);
  const format = value => new Intl.DateTimeFormat('es-ES', { day: 'numeric', month: 'short' }).format(new Date(`${value}T12:00:00`));
  return `${format(weekStart)} – ${format(end)}`;
}
function weekStorageKey() {
  return state.athlete ? `runflow.coach.week.${state.athlete.id}` : 'runflow.coach.week';
}
function rememberSelectedWeek(weekStart) {
  state.selectedWeekStart = mondayFromDateString(weekStart);
  try { localStorage.setItem(weekStorageKey(), state.selectedWeekStart); } catch (_) {}
}
function storedWeekForAthlete(athleteId) {
  try { return localStorage.getItem(`runflow.coach.week.${athleteId}`); } catch (_) { return null; }
}
...
function renderSummary() {
  ...
  if ($('summaryWeekLabel')) $('summaryWeekLabel').textContent = `${weekRangeLabel(athlete.week.week_start)} · ${athlete.week.status === 'published' ? 'publicada' : 'borrador'}`;
...
function renderWeekEditor() {
  ...
  if ($('selectedWeekLabel')) {
    $('selectedWeekLabel').textContent = `${weekRangeLabel(week.week_start)} · ${week.status === 'published' ? 'Publicada' : 'Borrador'}`;
    $('selectedWeekLabel').className = `badge ${week.status === 'published' ? '' : 'pending'}`;
  }
...
async function loadAthlete(id, weekStart = null) {
  const requestedWeek = mondayFromDateString(weekStart || state.selectedWeekStart || storedWeekForAthlete(id) || isoMonday());
  const data = await api(`/api/coach/athletes/${id}?week_start=${requestedWeek}`);
  state.athlete = data.athlete;
  rememberSelectedWeek(state.athlete.week?.week_start || requestedWeek);
...
async function goToWeek(weekStart) {
  ...
  const target = mondayFromDateString(weekStart);
  rememberSelectedWeek(target);
  $('saveWeekStatus').textContent = 'Cargando semana…';
  try {
    await loadAthlete(state.athlete.id, target);
    switchView('week');
    $('saveWeekStatus').textContent = '';
  } catch (error) {
    $('saveWeekStatus').textContent = '';
...
async function refreshAthletes(selectId = null) {
  ...
  state.selectedWeekStart = storedWeekForAthlete(id) || state.selectedWeekStart || isoMonday();
  await loadAthlete(id, state.selectedWeekStart);
}
...
$('athleteSelect').addEventListener('change', () => { const id = $('athleteSelect').value; state.selectedWeekStart = storedWeekForAthlete(id) || isoMonday(); loadAthlete(id, state.selectedWeekStart).catch(error => showMessage(error.message, 'error')); });
...
$('weekStart').addEventListener('change', () => {
  const selected = mondayFromDateString($('weekStart').value);
  if ($('weekStart').value !== selected) $('weekStart').value = selected;
  goToWeek(selected);
});
$('previousWeek').addEventListener('click', () => goToWeek(addDays(state.athlete.week.week_start, -7)));
$('currentWeek').addEventListener('click', () => goToWeek(isoMonday()));
$('nextWeek').addEventListener('click', () => goToWeek(addDays(state.athlete.week.week_start, 7)));
...
$('saveWeek').addEventListener('click', async () => {
  try {
    ...
    state.athlete.week = data.week; rememberSelectedWeek(data.week.week_start); ensureWeek(); renderAll(); $('saveWeekStatus').textContent = 'Borrador guardado'; showMessage('La semana se ha guardado como borrador.', 'success');
...
$('publishWeek').addEventListener('click', async () => {
  try {
    ...
    state.athlete.week = data.week; rememberSelectedWeek(data.week.week_start); ensureWeek(); renderAll();
