(() => {
  ...
  function updateManualCopy(){
    ...
    const helpText='Registra una actividad que RunFlow no haya recibido de Intervals. RunFlow estimará su carga con tu historial cuando sea posible y marcará siempre las estimaciones como tales.';
    if(help&&help.textContent!==helpText)help.textContent=helpText;
    const preview=document.getElementById('v2LoadPreview');
    if(preview){const label=preview.querySelector('span');if(label&&label.textContent!=='Referencia interna sRPE')label.textContent='Referencia interna sRPE';const small=preview.querySelector('small');if(small&&small.textContent!=='La carga RunFlow final se calcula al guardar')small.textContent='La carga RunFlow final se calcula al guardar';}
  }
