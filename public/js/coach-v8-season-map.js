(() => {
  ...
  const days=(a,b)=>{const x=parse(a),y=parse(b);return x&&y?Math.max(1,Math.round((y-x)/86400000)+1):1};
  ...
  function guideMeta(notes){const out={};String(notes||'').split('\n').forEach(line=>{const i=line.indexOf(':');if(i<0)return;const k=line.slice(0,i).trim(),v=line.slice(i+1).trim();if(k==='Fase')out.phase=v;if(k==='Tipo guía')out.guideType=v;if(k==='Tipo')out.type=v;if(k==='Carga %')out.loadPct=v;if(k==='Rango')out.range=v});return out}

  function seasonRange(){
    const s=stateSafe()?.plan?.season,macros=allMacros(),goals=allGoals();
    let start=s?.start_date||macros.map(x=>x.start_date).filter(Boolean).sort()[0]||'';
    let end=s?.end_date||macros.map(x=>x.end_date).filter(Boolean).sort().slice(-1)[0]||goals.map(x=>x.goal_date).filter(Boolean).sort().slice(-1)[0]||'';
    if(!start){const d=new Date();start=iso(new Date(d.getFullYear(),d.getMonth(),1,12))}
    if(!end)end=addDays(start,364);
    if(String(end)<String(start))end=addDays(start,364);
    return{start,end};
  }
  function position(date,start,end){const total=days(start,end),offset=days(start,date)-1;return clamp(offset/Math.max(1,total-1)*100,0,100)}
  function width(a,b,start,end){const total=days(start,end);return Math.max(.8,clamp(days(a,b)/total*100,0,100))}
  function monthTicks(start,end){const a=parse(start),b=parse(end),out=[];if(!a||!b)return out;let d=new Date(a.getFullYear(),a.getMonth(),1,12);while(d<=b){out.push({date:iso(d),label:new Intl.DateTimeFormat('es-ES',{month:'short',year:d.getFullYear()!==a.getFullYear()?'2-digit':undefined}).format(d)});d=new Date(d.getFullYear(),d.getMonth()+1,1,12)}return out}
  ...
  function render(){
    const host=q('#weekView');
    if(!host)return;
    let box=q('#v8SeasonMap');
    if(!box){box=document.createElement('section');box.id='v8SeasonMap';box.className='card v8-season-map';const workspace=q('.calendar-workspace',host);host.insertBefore(box,workspace||host.firstChild)}
    ...
    const range=seasonRange(),ticks=monthTicks(range.start,range.end);
    const today=new Date().toISOString().slice(0,10),todayPos=position(today,range.start,range.end);
  ...
  const observer=new MutationObserver(()=>{if(q('#weekView.active')||q('#weekView'))render()});
  observer.observe(document.documentElement,{subtree:true,childList:true});
  q('#athleteSelect')?.addEventListener('change',()=>setTimeout(render,120));
