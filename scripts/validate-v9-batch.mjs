...
for(const marker of ["location.replace('/login')",'/js/coach-v9-stepwise-final.js','/js/coach-v9-session-generator-fix.js?v=9.5.2','/js/coach-v9-manual-planning.js','/js/coach-v9-contextual-recommender-v2.js','/js/coach-v9-plan-v2-import.js','/js/coach-learning.js?v=1.0.0','/css/coach-v9-plan-v2-import.css','/coach-base.html'])if(!primaryCoach.includes(marker))throw new Error(`Coach principal: falta ${marker}`);
...
for(const marker of ['Muy fatigado','Muy recuperado','daily-checkin','pending-feedback','Media personal'])if(!athleteLearning.includes(marker))throw new Error(`Check-in Athlete incompleto: ${marker}`);
const generator=fs.readFileSync(path.join(root,'public/js/coach-v9-session-generator-fix.js'),'utf8');
for(const marker of ['dur>60','isLongSession','targetMinutes','duration_mix','library_id'])if(!generator.includes(marker))throw new Error(`Política de duración/generación incompleta: ${marker}`);
...
console.log(`OK: 307/307 fuente, ${over60.length} no-largas >60 excluidas operativamente, bandas 45=${band45} / 60=${band60}, aprendizaje Athlete + Coach validado.`);
